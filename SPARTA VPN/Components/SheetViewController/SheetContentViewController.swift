...
public class SheetContentViewController: UIViewController {
    ...
    public private(set) var detectedScrollView: UIScrollView?
    ...
    public var treatPullBarAsClear: Bool = SheetPresentationController.treatPullBarAsClear {
        didSet {
    ...
    private var topContainerZeroHeightConstraint: NSLayoutConstraint?
    ...
    func updatePreferredHeight(keyboardIsHidden: Bool = true) {
        ...
        if keyboardIsHidden != self.keyboardIsHidden && !keyboardIsHidden {
            self.keyboardIsHidden = false
            safeAreaHeight = self.childViewController.view.safeAreaBottomHeight
        } else if keyboardIsHidden && !self.keyboardIsHidden {
        ...
        self.preferredHeight = self.contentView.systemLayoutSizeFitting(
            fittingSize,
    ...
    private func setupChildViewController() {
        ...
        self.detectedScrollView = detectScrollView(subviews: self.childViewController.view.subviews)
        ...
        self.contentView.backgroundColor = self.childViewController.view.backgroundColor
    ...
    private func setupContentView() {
        ...
        self.contentView.addSubview(topContainer) {
            ...
            $0.left.pinToSuperview()
            $0.right.pinToSuperview()
            self.topContainerZeroHeightConstraint = $0.height.set(0)
        }
        
        self.contentView.addSubview(contentWrapperView) {
            $0.top.align(with: self.topContainer.al.bottom)
            $0.left.pinToSuperview()
            $0.right.pinToSuperview()
            $0.bottom.pinToSuperview()
        }
        ...
        self.contentView.layer.compatibleMaskedCorners = [.layerMaxXMinYCorner, .layerMinXMinYCorner]
    ...
    private func detectScrollView(subviews: [UIView]) -> UIScrollView? {
        var scrollView: UIScrollView?
        if let tableView = subviews.first(where: { $0 is UITableView }) as? UIScrollView  {
            scrollView = tableView
        } else if let collectionView = subviews.first(where: { $0 is UICollectionView }) as? UICollectionView,
                  let layout = collectionView.collectionViewLayout as? UICollectionViewFlowLayout,
                  layout.scrollDirection == .vertical {
            scrollView = collectionView
        } else if let tableView = subviews.first?.subviews.first(where: { $0 is UITableView }) as? UIScrollView {
            scrollView = tableView
        } else {
            scrollView = subviews.first { $0 is UIScrollView } as? UIScrollView
        }
        return scrollView
    }
    ...
    @objc
    private func pullBarTapped(_ gesture: UITapGestureRecognizer) {
